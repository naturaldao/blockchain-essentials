...
我们称比特币的这一特性为抗审查（anti-censorship）。在比特币的实际应用中，抗审查直接体现在没有人可以伪造、修改、撤销或废除你的交易——所以也请注意，这里的抗审查指的是比特币交易不可纂改，而并非某件事没有门槛没有限制的那种抗审查。

抗审查被广泛接受为区块链的核心规则和核心价值观，即区块链里的数据不可被伪造、篡改或删除。抗审查常常被直接理解为**区块链数据不可纂改**——请特别注意，我这里说的是区块链数据，也就是区块链原生的数据（如转账数据），但并非指区块链里面已有的一切数据。或者说，请不要认为只要是区块链里面的数据，就不可纂改就百分百可信，因为数据进入区块链之前是可以造假的！我们无法用区块链技术保证非区块链原生数据的真实可靠性。譬如一个群体的身份信息。

理论上，如果整个社区达成了共识，可以通过回滚做到撤销或者废除交易，甚至达到删除帐户的目的。但是实际上，这很容易引发信任危机乃至社区的分裂！
