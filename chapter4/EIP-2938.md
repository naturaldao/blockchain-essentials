...
EIP-7702 提出了一种同时接受 contract\_code 和签名字段的新交易类型，在开始执行交易时，它将签名者账户的合约代码设置为 contract\_code。在交易结束时，它会将代码重新设置为空。
...
它与EIP-4337 构建的所有账户抽象工作高度兼容，“用户需要签名的合约代码实际上可以是现有的 EIP-4337 钱包代码”。
