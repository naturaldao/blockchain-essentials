...
本协议定义了智能合约的接口，可用于管理多种代币。利用本协议只需部署一个智能合约便可管理任意种同质代币、非同质代币或其它代币（如半同质代币）的组合。

Abstract 前言

This standard outlines a smart contract interface that can represent any number of fungible and non-fungible token types. Existing standards such as ERC-20 require deployment of separate contracts per token type. The ERC-721 standard’s token ID is a single non-fungible index and the group of these non-fungibles is deployed as a single contract with settings for the entire collection. In contrast, the ERC-1155 Multi Token Standard allows for each token ID to represent a new configurable token type, which may have its own metadata, supply and other attributes.

本协议给出了一种智能合约接口，能定义任意种类和数量的同质和非同质代币。在现有的代币标准中，ERC-20标准在使用时，需要对不同的代币种类部署不同的智能合约；ERC-721标准在使用时，需要用Token ID给每一个非同质代币的一个唯一的索引（编号），并把一组这样的非同质代币作为一个集合部署在一个智能合约中。而在ERC-1155多代币标准中，Token ID有新的用途，用它定义的代币种类，可以有自己的元数据（metadata），发行量及各种属性。

The \_id argument contained in each function’s argument set indicates a specific token or token type in a transaction.

本协议各个函数中的参数”\_id”表示交易中的某个代币或代币种类。

Motivation动机

Tokens standards like ERC-20 and ERC-721 require a separate contract to be deployed for each token type or collection. This places a lot of redundant bytecode on the Ethereum blockchain and limits certain functionality by the nature of separating each token contract into its own permissioned address. With the rise of blockchain games and platforms like Enjin Coin, game developers may be creating thousands of token types, and a new type of token standard is needed to support them. However, ERC-1155 is not specific to games and many other applications can benefit from this flexibility.

ERC-20代币标准要求每个代币对应一个智能合约，ERC-721代币标准要求每个非同质代币集合对应一个智能合约。这两个标准定义的每个代币合约都有自己的地址，导致以太坊上承载的代码臃肿不堪，并且限制了某些功能。随着区块链游戏及平台（如Enjin）的崛起，游戏开发者很有可能需要创建成千上万种代币，这迫切需要一个全新的代币标准。ERC-1155便应运而生，它非常灵活，不仅适用于游戏，也适用于很多其它领域的应用。

New functionality is possible with this design such as transferring multiple token types at once, saving on transaction costs. Trading \(escrow / atomic swaps\) of multiple tokens can be built on top of this standard and it removes the need to “approve” individual token contracts separately. It is also easy to describe and mix multiple fungible or non-fungible token types in a single contract.

ERC-1155将会催生很多新的功能，比如一次性转账多种代币，从而节省转账费用。该协议也支持多种代币交易（中介交易escrow/原子互换atomic swap）并省却了以往那些交易中需要每个合约进行授权的琐碎步骤，它可以轻易地在一个合约中描述和处理多种同质代币、非同质代币或两者混合的情况。
...
**实现ERC-1155标准的智能合约必须实现ERC1155接口定义的所有函数**。
...
ERC-1155 Token Receiver ERC-1155代币接收者
...
如果交易的代币是用其它合约标准实现的并且代币的实现代码中包含标准和非标准函数，则该交易可以遵循该代币的合约标准而不是ERC1155标准。详细信息请参看“Compatibility with other standards “。
...
场景8：你写了一个智能合约，实现了ERC1155TokenReceiver接口函数的功能，你调用onERC1155Received或onERC1155BatchReceived或两个都调用，把该合约代币转账到另一个地址。
...
在此场景下，如果合约设计者希望合约仍然保留对该代币的所有权，则代币所有权可以（并非必须）仍然为合约所有。
...
场景9：用户通过非标准API函数转账代币。所谓的非标准API函数是指除safeTransferFrom和safeBatchTransferFrom以外的API函数。
...
调用者必须得到授权才可以从参数“\_from”指定的账户转账代币。
...
对参数“token\_id”所指的代币，如果其转出账户的余额小于参数“\_value”所定义的金额，则交易必须回滚。
...
调用者必须得到授权才可以从参数“\_from”设定的账户转账代币。
...
对参数“\_ids”所指的代币，如果其任一转出账户的余额小于该交易所对应的参数“\_values”所定义的金额，则交易必须回滚。
...
转账交易的代币种类由参数“\_id”所指代。

。The \_value argument MUST be the number of tokens the holder balance is decreased by and match what the recipient balance is increased by.

在转账交易中，一方余额减少而另一方余额增加。余额减少方所减少的代币数量必须等于参数“\_value”的值，并且也必须等于余额增加方所增加的代币数量。

。When minting/creating tokens, the \_from argument MUST be set to 0x0 \(i.e. zero address\).

当挖矿/创建代币时，参数“\_from”的值必须设为“0x0”（即零地址）。
...
当消除/销毁代币时，参数”\_to”的值必须设为“0x0”（即零地址）。
...
转账交易的一系列代币中每一种代币的索引都必须对应数组参数“\_ids”中的一个值，即数组参数“\_ids”包含了所有转账代币的索引。

。The \_values array argument MUST contain the number of token to be transferred for each corresponding entry in \_ids.

转账交易的一系列代币中，每一种代币的转账数量都必须对应数组参数“\_values”中的一个值，即数组参数“\_values”包含了所有转账代币中每一种代币的转账数量。
...
给定一个代币ID，根据TransferSingle和TransferBatch事件观测到的从地址“0x0”转出的代币总量减去发送至地址“0x0”的代币总量所得到的数量可被用户及交易所视为该代币的“流通总量”。

**·** To broadcast the existence of a token ID with no initial balance, the contract SHOULD emit the TransferSingle event from 0x0 to 0x0, with the token creator as \_operator, and a \_value of 0.

给定一个代币ID，如果该代币的初始发行量为0，则其合约触发TransferSingle事件时，应从地址“0x0”发送到地址“0x0”，代币的创造者（creator）由参数“\_operator”所定义，参数“\_value”的值为0。
...
当发生事件重入（re-entry）时（比如一个合约在收到代币后紧接着转发代币）为了保证事件触发顺序的正确，系统的状态（？state balance）和事件（？events balance）必须匹配，然后再调用外部合约。
...
在转账交易中，一方余额减少而另一方余额增加。余额减少方所减少的每一种代币的数量必须等于参数“\_values”中的一个值，并且也必须等于余额增加方所增加的该种代币的数量。
...
当一个API函数是用于向一个智能合约转账ERC-1155代币时，如果该智能合约实现了ERC1155TokenReceiver接口函数，则调用safeTransferFrom或safeBatchTransferFrom所必须遵循的规则仍然必须遵循。如果该合约没有实现ERC1155TokenReceiver接口函数则交易要回滚，但在交易回滚后可以（并非一定）继续执行后续的语句。
...
myTransferFrom会将所有”\_ids”和“\_values”对应的代币种类及数量从”\_from”地址转账到“\_to”地址。

3 myTransferFrom emits TransferBatch with the details of what was transferred from address \_from to address \_to.

myTransferFrom会触发TransferBatch事件，事件会包含所转账的代币信息、发送者和接收者地址。
...
此时myTransferFrom应该立刻回滚交易，因为onERC1155BatchReceived函数并不显式确保代币一定被成功接收。
...
注意：如果接收方地址无法接收ERC-1155代币，则这种操作可能会导致转账的代币永久丢失。
...
仅当调用非标准函数时才允许代币被发送给一个没有实现ERC1155TokenReceiver接口函数的智能合约。在这种情况下，safeTransferFrom和safeBatchTransferFrom必须回滚交易（除非它包含标准和非标准的接口实现，详细信息请参看“Compatibility with other standards “）。
...
对一个初始发行量为0的代币，给定其token ID，其合约在触发TransferSingle事件时发送方地址和接收方地址都应该设为“0x0”，代币的创建者（creator）为参数“\_operator”，参数“\_value”值为0。
...
注意：此规则也适用于初始发行量不为0的代币。对初始发行量不为0的代币其合约账户的余额也必须在触发的事件中反映出来，也即合约账户的初始余额（比如在构造函数中设定的余额）必须在触发的事件中反映出来。
...
综上所述，挖矿/创建和消除/销毁代币的操作是特殊的转账交易，有可能通过除safeTransferFrom和safeBatchTransferFrom以外的用户自定义函数实现。当用户用自定义的函数实现时，必须遵循“Implementation specific transfer API rules”章节所定义的规则。
...
当进行挖矿/创建和消除/销毁代币的操作时，即便用户使用的函数并非一定安全，或用户使用的函数可能既包含自定义函数也包含标准函数，其触发的挖矿/创建和消除/销毁事件也必须遵循上述规则。
...
如果挖矿操作是给自己转账代币，则合约可不用执行ERC1155TokenReceiver的接口函数。除此以外在所有的其它情形中，ERC1155TokenReceiver的规则都必须遵循。
...
注意：对一个代币标准的合约实现，建议要么都用标准函数要么都用自定义函数，把两者混在一起反而不好。但对ERC-1155/ERC-721代币标准，我们提供了一个实例，该实例代码中既包含标准函数也包含用户自定义函数。实例代码的链接在参考章节中。
...
在设计ERC-1155代币标准时有一个重要的考虑：即使ERC-1155代币在交易过程中遵循的是其它代币标准，但ERC-1155所定义的事件必须遵循ERC-1155的规则触发。

Metadata 元数据

The URI value allows for ID substitution by clients. If the string {id} exists in any URI, clients MUST replace this with the actual token ID in hexadecimal form. This allows for a large number of tokens to use the same on-chain string by defining a URI once, for that large number of tokens.

用户可以用ID值取代URI值。如果URI中包含{id}字串，则用户必须将其用十六进制数的真实代币ID取代。这可以让大量代币共用相同的链上字符串，只需要定义一次URI即可。

**·** The string format of the substituted hexadecimal ID MUST be lowercase alphanumeric: \[0-9a-f\] with no 0x prefix.

代币的ID必须仅仅包含小写字母和数字【0-9a-f】并且不带前缀“0x”。

**·** The string format of the substituted hexadecimal ID MUST be leading zero padded to 64 hex characters length if necessary.

代币的ID必须包含64个十六进制字符，若字符数没有64个，则在字符串前面全部用0填充。
...
URI实例：“https://token-cdn-domain/{id}.json”中有{id}字串，它被实际的代币ID（代币ID为“314592/0x4CCE0”）取代后为：
...
URI函数不能用于检查某个代币是否存在，因为存在这种可能性：一个代币合约的实现代码对一个不存在的代币仍然返回一个有效的字符串。
...
JSON框架大体上基于“ERC721 Metadata JSON Schema”，但还包括一些可选的格式让用户可以用ID进行替代。如果JSON值里面包含{id}字串，它必须用真实的代币ID替代，其规则如下：
...
下例为一个ERC-1155 Metadata JSON文件。其属性数组罗列了一些建议使用的格式，这些格式定义了代币的显示属性和元数据。
...
setApprovalForAll函数让授权者授权其他操作者管理其所有的代币。如果授权者只希望授权部分代币的管理权限则可以使用ERC-1761 Scoped Approval接口。如果想查看setApprovalForAll函数设置的授权状态可以调用isApprovedForAll函数进行查询。
...
无论代币是否被授权给他人管理，代币的所有者永远都能管理自己的代币。因此代币的所有者无需调用setApprovalForAll给自己授权就可直接管理代币。
...
本标准支持safeTransferFrom和onERC1155Received函数，因为对单一代币的交易调用它们所需的耗费少很多，这应该好理解。
...
随着以太坊生态系统的持续发展，很多DAPP依赖传统的数据库和浏览器提供的API服务获取和分类数据。ERC-1155标准保证其智能合约所触发的事件日志能提供足够多的数据，准确地记录所有代币账户余额的变化。对合约中每一个ERC-1155代币，可以用数据库或浏览器监听所触发的事件，也可以提供经过索引和分类的搜索结果。
...
setApprovalForAll函数让授权者授权其他操作者管理其所有的代币。它使得交易所交易和合约交易之间完全无障碍。
...
如果授权者只希望授权部分代币，代币的部分数量或其它权限的管理则可以另外使用一个接口或外部合约来处理。这样做的基本原理是保证ERC-1155标准尽量通用，在任何情况下都无需在具体的代码实现中编写特殊的授权机制。而且这些具体的代码实现可能根本就不需要这种特殊的授权机制。如果实在需要这种授权可以使用标准的代币授权接口比如ERC-1761 Scoped Approval接口，它和ERC-1155协议是兼容的。
...
本标准可用于定义一个领域内的多种代币类型。同质代币和非同质代币可以共存于同一个智能合约内。
...
safeBatchTransferFrom函数可以同时转账各种数量的各种代币。ERC-1155的设计支持批量转账而无需另外的打包合约（wrapper contract），而现有的其它代币标准要进行批量转账则需要另外的打包合约（wrapper contract）。当系统需要转账多种代币时，传统的做法不得不一笔笔的转账，而ERC-1155的设计使得转账多种代币所需的gas耗费大为降低，。
...
建议用户和钱包项目方要进行批量转账时，对待转账的所有token ID和每个ID对应的代币数量进行排序（升序排列）。因为ERC-1155标准对进行了排序的代币进行批量转账时会显著降低gas耗费。详细信息请参看“Horizon Games - Multi-Token Standard”章节中的“打包余额（packed balance）”实例。
...
balanceOfBatch函数让用户仅仅只需调用这个函数便能获取多个代币所有者及其所持有的代币的余额信息。
...
为了让ERC-1155合约的实现尽量少地占用存储，枚举（枚举所有的代币ID及每个代币对应的数量）必须用事件日志实现。建议交易所和区块链浏览器项目方在本地保留一个数据库至少记录token ID，供应量和URI这三项信息。数据库的数据可以从TransferSingle，TransferBatch和URI事件中提取，记录的时间可以从智能合约部署的那一刻算起一直记录到最新产生的区块。
...
ERC-1155合约的编写者在代币被创建，挖矿，交易和销毁时必须小心谨慎地触发TransferSingle和TransferBatch事件。

Non-Fungible Tokens 非同质代币
...
下列示范展示了如何在一个智能合约中既定义同质代币也定义非同质代币，但ERC-1155标准并不强制要求编写者必须按该示范编写。
...
在任何一个ERC-1155函数中，参数”uint256 \_id”的前128位都可以（并非一定）被用来定义token ID。而余下的128位可以（并非一定）被用来定义非同质代币的唯一索引。
...
非同质代币可以用带索引的变量访问。如果一个合约的代币集合中有某个非同质代币，用户可以用”\_id”访问该非同质代币，此时”\_id”值的构成为&lt;uint128:非同质代币ID&gt;&lt;uint128:非同质代币的索引&gt;。
...
为了标识一个非同质代币集合/类别，你可以将参数“\_id”的值设为&lt;uint128:非同质代币ID&gt;&lt;uint128:0&gt;。如果合约的代码实现使用了这个技巧，则意味着一个非同质代币的索引基于“1”（1-based）。
...
在合约中，访问一个非同质代币需要两部分数据（译者注：即高128位数据和低128位数据），这两部分数据可以分别用uint128\(~0\)和128位移位运算获得。
...
注意：128不是个固定值，合约编写者可以自己选择合适的方式来划分这256位的用途。用户只需要能从合约触发的事件中看到转账交易和挖矿发生就行了，当然用户也可以通过这些事件追踪余额的变化。如果用户希望从ID中判断转账的代币类型（同质代币或非同质代币）则需要研究合约的实现代码是如何划分ID数位的用途的。

The ERC-1155 Reference Implementation is an example of the split ID bits strategy.

ERC-1155参考案例展示了如何划分ID的数位用途。

Natural Non-Fungible tokens 自然的非同质代币
...
有一个简单表示非同质代币的方法：对每一个非同质代币尽量用“1”来定义。这是对真实世界最自然的映射。在真实世界中每一个唯一的事物有且仅有一个，而每一种同质代币则有多个。
