...
通证（token）是个神奇的名词，它是区块链硬核之一，但对于中国人来说，也是多数人还没有搞明白的一个新词！

源自古英格兰的盎格鲁撒克逊语（Anglo-Saxon）的token是个概念和功用范畴都很大的单词：它可以是代币、代金券、赠券、令牌、奖牌或者标识物、纪念物等等。总的来说，token的原意是以一样东西代表或者象征另一样东西，后者可以是物质的也可以是精神的，可以是实实在在的也可以是虚拟的。
...
区块链项目里面所使用的通证，它通常同时涵盖了以上多种属性（功用）。区块链里的通证完全可能集代币、代金券、荣誉标识、身份标识暨确权工具、价值量化指标、系统通行证和安全保护于一身！
...
**通证，就是通过共识规则、智能合约、应用目标等建立起来的集货币属性、价值属性、安保属性、确权属性等等多种属性于一体的区块链凭证。**
