...
token在一千多年的时间里没有完全同义的中文字词，直到2017年11月才由CSDN副总孟岩翻译为“通证”。

总的来说，通证（token）的原意是以一样东西代表或者象征另一样东西，后者可以是物质的也可以是精神的，可以是实实在在的也可以是虚拟的。token现在已经广泛应用到我们生活的方方面面：
...
所以我们通常从货币的角度所谈的比特币其实就是一种以货币属性为主的通证，但比特币决非货币，区块链里面也不存在货币，它里面的“币”全都是通证（token）。并且，不是所有的通证都具有货币属性！譬如我们在DAO的领域实践的声望（Reputation），它是一种独特的通证，因为无法转账，完全不具备货币属性！
